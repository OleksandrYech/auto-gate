...
try:
    ...
    class DigitalInputDevice:
        def __init__(self, pin, pull_up=None, pin_factory=None):
            self.pin, self.is_active = pin, False
            logging.getLogger(__name__).info(f"Мок DigitalInputDevice створено для піна {pin}")

        def close(self): pass

        def wait_for_active(self, timeout=None): return True


class ReedSwitch:
    """Клас для роботи з герконовим датчиком, який активний (замкнений), коли ворота ВІДКРИТІ."""
    ...
    def __init__(self, pin_number: int, name: str = "ReedSwitch"):
        ...
        try:
            # pull_up=True означає, що коли контакти геркона замикаються на землю, is_active стає True.
...
class SensorManager:
    """Керує лише герконовим датчиком."""
