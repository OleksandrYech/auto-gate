...
# ... (функція _check_bbox_roi_intersection залишається без змін) ...
...
class CVProcessor:
    def __init__(self,
        ...
        if self.plate_session:
            ...
            self.plate_model_input_height, self.plate_model_input_width = shape[2], shape[3]

        self.roi_config = self._load_roi_config(roi_config_path)
        self._logger.info("CVProcessor успішно ініціалізовано.")

    # ... (інші методи, як _format_plate_number, recognize_plate_characters, _load_onnx_model і т.д.
    #      залишаються без змін, оскільки вони реалізовані добре) ...
    ...
    def get_plate_number_from_image(self, image_bgr: np.ndarray, cam_type: str, save_intermediate_steps: bool = False,
        ...
        """
        Основна функція, що виконує повний цикл детекції та розпізнавання.
        ДОДАНО: Більш детальне логування кожного кроку для легшого налагодження.
        """
        timestamp = time.strftime('%Y%m%d_%H%M%S')
        final_image_path = None

        # Крок 1: Детекція автомобіля
        vehicle_boxes = self.detect_vehicle_in_frame(image_bgr, cam_type)
        if not vehicle_boxes:
            # Це не помилка, а штатна ситуація, тому використовуємо рівень INFO
            self._logger.info(f"[{cam_type.upper()}] Крок 1/4: Автомобілі в зоні ROI не виявлено.")
            return None, None
        self._logger.info(f"[{cam_type.upper()}] Крок 1/4: Виявлено {len(vehicle_boxes)} авто. Вибираємо найбільше.")
        ...
        # Крок 2: Детекція номерного знака на зображенні авто
        plate_boxes = self.detect_license_plate(vehicle_image)
        if not plate_boxes:
            self._logger.info(f"[{cam_type.upper()}] Крок 2/4: Номерний знак на автомобілі не виявлено.")
            return None, None
        self._logger.info(f"[{cam_type.upper()}] Крок 2/4: Виявлено номерний знак.")

        plate_box = max(plate_boxes, key=lambda b: (b[2] - b[0]) * (b[3] - b[1]))
        # Додаємо невеликі відступи для кращого розпізнавання
        h, w = vehicle_image.shape[:2]
        x1, y1, x2, y2 = plate_box
        padding_x, padding_y = int((x2 - x1) * 0.05), int((y2 - y1) * 0.2)
        padded_plate_box = (max(0, x1 - padding_x), max(0, y1 - padding_y), min(w, x2 + padding_x),
                            min(h, y2 + padding_y))
        ...
        # Крок 3: Розпізнавання символів (OCR)
        raw_plate_text = self.recognize_plate_characters(plate_image)
        if not raw_plate_text:
            self._logger.info(f"[{cam_type.upper()}] Крок 3/4: OCR не зміг розпізнати символи на номерному знаку.")
            return None, None
        self._logger.info(f"[{cam_type.upper()}] Крок 3/4: OCR розпізнав текст: '{raw_plate_text}'.")

        # Крок 4: Форматування та валідація
        formatted_plate = self._format_plate_number(raw_plate_text)
        if not formatted_plate:
            self._logger.info(f"[{cam_type.upper()}] Крок 4/4: Номер '{raw_plate_text}' не пройшов валідацію.")
            return None, None

        self._logger.info(f"[{cam_type.upper()}] Крок 4/4: УСПІХ! Розпізнано та валідовано номер: '{formatted_plate}'")
        if save_intermediate_steps:
    ...
    # Я їх приховав для стислості, але вони мають бути у файлі
    def _format_plate_number(self, raw_text: str) -> Optional[str]:
        if not raw_text: return None
        clean_text = re.sub(r'[^A-Z0-9]', '', raw_text.upper())
        if len(clean_text) != 8:
            self._logger.debug(f"Номер '{clean_text}' відхилено через довжину ({len(clean_text)}).")
            return None
        plate_chars = list(clean_text)
        digit_to_letter, letter_to_digit = {'0': 'O', '1': 'I', '8': 'B'}, {'O': '0', 'I': '1', 'B': '8', 'A': '4'}
        for i in [0, 1, 6, 7]:
        ...
        self._logger.warning(f"Номер '{raw_text}' -> '{corrected_plate}' не відповідає стандарту AA1111AA.")
        return None
    ...
    def recognize_plate_characters(self, plate_image: np.ndarray) -> Optional[str]:
        try:
            ...
            result = self.ocr_reader.readtext(enhanced_plate, detail=0, paragraph=True, allowlist=self.OCR_ALLOWLIST)
            return "".join(result).replace(" ", "").upper() if result else None
    ...
    def _preprocess_for_mobilenet(self, image_bgr: np.ndarray) -> np.ndarray:
        return np.expand_dims(cv2.resize(image_bgr, (300, 300)), axis=0)
    ...
    def _parse_plate_detections(self, raw_output: np.ndarray, original_shape: tuple) -> List[Tuple[int, int, int, int]]:
        ...
        for det in raw_output[0]:
            ...
            if score > self.plate_confidence_thresh and class_id == LICENSE_PLATE_CLASS_ID:
                detections.append(tuple(
                    map(int, (det[0] * original_w, det[1] * original_h, det[2] * original_w, det[3] * original_h))))
        return detections
    ...
    def detect_vehicle_in_frame(self, image_bgr: np.ndarray, camera_type: str) -> List[Tuple[int, int, int, int]]:
        ...
        for i in range(len(scores[0])):
            if scores[0][i] > self.vehicle_confidence_thresh and int(classes[0][i]) in TARGET_VEHICLE_CLASS_IDS:
                box = boxes[0][i]
                all_detections.append((int(box[1] * w), int(box[0] * h), int(box[3] * w), int(box[2] * h)))
        roi_settings = self.roi_config.get(f"{camera_type}_camera_roi")
        return [bbox for bbox in all_detections if
                _check_bbox_roi_intersection(bbox, roi_settings)] if roi_settings else all_detections
